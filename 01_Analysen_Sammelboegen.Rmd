...
 \newline
Analyse von Freitexteintragungen unter Sonstige. Nur ausgewertet, wenn Hinweise auf Sprach- und Sprechstörungen entweder im Rahmen der Anamnese oder im Rahmen der Orientierenden Beurteilung der Entwicklung angegeben, die Frage nach Einleitung von diagnostischen oder therapeutischen Maßnahmen bejaht & die Option Sonstige unter diagnostische/therapeutische Maßnahme angekreuzt wurde. 

```{r sprache_freitext, message=FALSE, warning=FALSE}
### Fälle filtern
tbl1 <- u5 %>% filter((F12==1 | F13==1) & F15==1 & F20==1) %>% mutate(id=paste0(id, U, kind)) %>% select(id)
tbl2 <- u6 %>% filter((F14==1 | F15==1) & F17==1 & F22==1) %>% mutate(id=paste0(id, U, kind)) %>% select(id)
tbl3 <- u7 %>% filter((F12==1 | F13==1) & F15==1 & F20==1) %>% mutate(id=paste0(id, U, kind)) %>% select(id)
tbl4 <- u7a %>% filter((F21==1 | F22==1) & F24==1 & F29==1) %>% mutate(id=paste0(id, U, kind)) %>% select(id)
tbl5 <- u8 %>% filter((F32==1 | F33==1) & F35==1 & F40==1) %>% mutate(id=paste0(id, U, kind)) %>% select(id)
tbl6 <- u9 %>% filter((F21==1 | F22==1) & F24==1 & F29==1) %>% mutate(id=paste0(id, U, kind)) %>% select(id)
...
txt2 <- txt %>% filter(id %in% tbl$id) %>% # txt nach Fällen filtern: n = 995 Kinder mit Freitexten
  pivot_longer(cols = starts_with('Codes'), names_to = 'pos', values_to = 'Code') %>%
  na.omit() 

#txt2 %>% filter(pos == 'Codes1') %>% tabyl(U)

txt2 <- txt2 %>% 
  group_by(U, Code) %>% tally() %>% 
