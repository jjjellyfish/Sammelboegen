...
a <- tbl %>% filter(x %in% c('ja', 'nein')) %>% mutate(y='alle U-Untersuchungen') #für Abbildung
...
p <- tmp %>% 
  filter(x == 'ja') %>%
  ggplot(aes(y=perc, x=y, fill=y, label = scales::percent(perc, accuracy=.1, decimal.mark = ","))) + 
  ...
  scale_fill_manual(values=c('#00386a', '#BED600', '#eda63b')) +
  facet_wrap(~ factor(U, levels=c('U4', 'U5', 'U6', 'U7', 'ges.')), nrow=1) +
  ylab('Anteil U-Untersuchungen') +
  theme_bw() +
...
Falls Prüfung mittels Tonschwellenaudiometrie NICHT durchgeführt: Sonstige Gründe gegen die Durchführung (Freitextantworten, als Nenner wird die Gesamtzahl von validen Antworten auf die Frage nach Gründen gegen die Durchführung einer Tonschwellenaudiometrie verwendet)
