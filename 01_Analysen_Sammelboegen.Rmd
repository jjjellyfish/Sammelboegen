---
title: "Ergebnisse aus den U-Sammelbögen"
output: 
  pdf_document:
...
kbl2 = function(...) {
  kableExtra::kbl(..., format.args = list(decimal.mark = ',', big.mark = "."), linesep = "")
}
...
# 3. Rücklauf der Erhebungen und Stichprobencharakterisierung

## 3.1 Erhebungen in den Arztpraxen

### 3.1.1 Arztfragebogen und U-Sammelbögen
...
psych::describe(tbl$n)
```


## 3.2 Erhebungen bei den Eltern

### 3.2.1 Elternfragebogen

Studienunterlagen an Eltern ausgeteilt - Kreuz gesetzt 

```{r echo=FALSE, message=FALSE, warning=FALSE}
u2$F14 <- ifelse(u2$F14==0, 99, u2$F14)
tbl1 <- tbl_janein(u2$F14)
u3$F14 <- ifelse(u3$F14==0, 99, u3$F14)
tbl2 <- tbl_janein(u3$F14)
u4$F12 <- ifelse(u4$F12==0, 99, u4$F12)
tbl3 <- tbl_janein(u4$F12)
u5$F21 <- ifelse(u5$F21==0, 99, u5$F21)
tbl4 <- tbl_janein(u5$F21)
u6$F23 <- ifelse(u6$F23==0, 99, u6$F23)
tbl5 <- tbl_janein(u6$F23)
u7$F21 <- ifelse(u7$F21==0, 99, u7$F21)
tbl6 <- tbl_janein(u7$F21)
u7a$F30 <- ifelse(u7a$F30==0, 99, u7a$F30)
tbl7 <- tbl_janein(u7a$F30)
u8$F41 <- ifelse(u8$F41==0, 99, u8$F41)
tbl8 <- tbl_janein(u8$F41)
u9$F30 <- ifelse(u9$F30==0, 99, u9$F30)
tbl9 <- tbl_janein(u9$F30)
...
tbl %>% dplyr::select(Antwort, contains('[n]')) %>%
  kbl2(booktabs = T, centering=F, align=align_string(tbl)) %>%
  kable_styling(latex_options = "HOLD_position", position = "left") %>% 
...
# 4. Ergebnisse der Evaluation von Abschnitt B (Früherkennungsuntersuchungen) der Kinder-Richtlinie 

## 4.1	Untersuchung der Augen (U2-U9)

### 4.1.2 Inspektion und Prüfung im durchfallenden Licht (U2 und U3)

#### 4.1.2.1 Häufigkeit der Durchführung 
...
#### 4.1.2.2	Häufigkeit der Feststellung von Auffälligkeiten
...
##### 4.1.2.3	Häufigkeit von Überweisungen
...
### 4.1.3	Brückner-Test (U4–U7)

#### 4.1.3.1	Häufigkeit der Durchführung
...
#### 4.1.3.2	Häufigkeit der Feststellung von Auffälligkeiten
...
bmp(file="04_Abbildungen/Transilluminationsunterschiede.bmp", width = 750, height = 500, type="windows")
print(p)
dev.off()
...
### 4.1.4	Inspektion, Pupillenstatus, Hornhautreflexbildchen, Stereo-Test und Formenwiedererkennungstest (U7a–U9)

#### 4.1.4.1	Häufigkeit der Durchführung 
...
Anteil der Kinder in der U7a, U8 bzw. U9, bei denen keiner, einer, zwei, drei, etc. der Sehtests durchgeführt wurden.
```{r message=FALSE, warning=FALSE}
augen_recode <- function(x) {
  case_when(
    x == 1 ~ 1, 
    x == 2 ~ 0,
    x == 0 ~ as.numeric(NA)
...
U7a_tmp <- u7a %>% 
  mutate_at(vars(F9, F11, F13, F16, F19), augen_recode) %>% 
  select(X, F9, F11, F13, F16, F19) %>% 
  group_by(X) %>%
  mutate(sum_test_u7a=sum(F9 + F11 + F13 + F16 + F19, na.rm=TRUE),
         sum_test_u7a=ifelse(is.na(F9)|is.na(F11)|is.na(F13)|is.na(F16)|is.na(F19), 
                             NA, sum_test_u7a)) %>% 
  ungroup() %>%
  count(sum_test_u7a)
...
U8_tmp <- u8 %>% 
  ...
  select(X, F9, F11, F13, F16, F19) %>% 
  group_by(X) %>%
  mutate(sum_test_u8=sum(F9 + F11 + F13 + F16 + F19, na.rm=TRUE),
         sum_test_u8=ifelse(is.na(F9)|is.na(F11)|is.na(F13)|is.na(F16)|is.na(F19), 
                             NA, sum_test_u8)) %>%   
  ungroup() %>%
  count(sum_test_u8)
...
U9_tmp <- u9 %>% 
  ...
  select(X, F9, F11, F13, F16, F19) %>% 
  group_by(X) %>%
  mutate(sum_test_u9=sum(F9 + F11 + F13 + F16 + F19, na.rm=TRUE),
         sum_test_u9=ifelse(is.na(F9)|is.na(F11)|is.na(F13)|is.na(F16)|is.na(F19), 
                             NA, sum_test_u9)) %>%   
  ungroup() %>%
  count(sum_test_u9)

tbl <- bind_cols(U7a_tmp, U8_tmp[2], U9_tmp[2]) 
names(tbl) <- c('tests', 'u7a', 'u8', 'u9')

# fehlende erstmal extrahieren
tmp <- tbl %>% filter(is.na(tests))
tbl <- tbl %>% filter(!is.na(tests))

tbl <- tbl %>%
  group_by(tests)%>% 
  mutate(ges = sum(u7a, u8, u9)) %>% 
  ungroup() %>% 
  mutate(u7a_perc = u7a / sum(u7a) * 100, 
         u8_perc = u8 / sum(u8) * 100, 
         u9_perc = u9 / sum(u9) * 100, 
         ges_perc = ges / sum(ges) * 100) %>% 
  select(tests, contains('u7a'), contains('u8'), contains('u9'), contains('ges')) %>% 
  janitor::adorn_totals('row', name='valides n')


# fehlende Werte manuell ergänzen
tbl[nrow(tbl) + 1,] <- 
  c('fehlend', tmp[2], NA, tmp[3], NA, tmp[4], NA, sum(tmp[2]+ tmp[3]+ tmp[4]), NA)

tbl[nrow(tbl) + 1,] <- 
  list('gesamt n', 5290, NA, 5349, NA, 5002, NA, 15641, NA)

names(tbl) <- c('Anzahl Sehtests', rep(c('n', '%'), 4))

tbl %>% kbl2(booktabs = T, centering=F, align=align_string(tbl), digits=1) %>%
  add_header_above(c('', 'U7a'=2, 'U8'=2, 'U9'=2, 'Ges.'=2)) %>% 
  kable_styling(latex_options = "HOLD_position", position = "left") %>% 
  row_spec(6, extra_css = "border-bottom: 1px solid", hline_after=TRUE)
```

  \linebreak

Anteil der Kinder in der U7a, U8 bzw. U9, bei denen die Hornhautreflexbildchen *nicht* geprüft wurden, aber der Stereo-Test oder der Formenwiedererkennungstest durchgeführt wurden. Nur ausgewertet, wenn Hornhautreflexbildchen *nicht* geprüft wurden.
```{r message=FALSE, warning=FALSE}
u7a <- u7a %>% mutate(test = ifelse(F16 == 1 | F19 == 1, '1', '2'), 
                      test = ifelse(F16 == 0 | F19 == 0, NA, test))
u8 <- u8 %>% mutate(test = ifelse(F16 == 1 | F19 == 1, '1', '2'), 
                    test = ifelse(F16 == 0 | F19 == 0, NA, test))
u9 <- u9 %>% mutate(test = ifelse(F16 == 1 | F19 == 1, '1', '2'), 
                    test = ifelse(F16 == 0 | F19 == 0, NA, test))


tbl1 <- tbl_janein(u7a$test[u7a$F13 == 2])
tbl2 <- tbl_janein(u8$test[u8$F13 == 2])
tbl3 <- tbl_janein(u9$test[u9$F13 == 2])
tbl <- bind_cols(tbl1, tbl2[2:3], tbl3[2:3])
...
Anteil der Kinder in der U7a, U8 bzw. U9, bei denen Stereo-Test UND Formenwiedererkennungstest durchgeführt wurden, NUR der Stereo-Test, NUR der Formenwiedererkennungstest oder KEINER von beiden Tests durchegführt wurde
```{r message=FALSE, warning=FALSE}
u7a <- u7a %>% mutate(test = ifelse(F16 == 1 & F19 == 1, '1', '2'), 
                      test = ifelse(F16 == 0 | F19 == 0, NA, test))
u8 <- u8 %>% mutate(test = ifelse(F16 == 1 & F19 == 1, '1', '2'), 
...
tbl1 <- tbl_janein(u7a$test)
tbl2 <- tbl_janein(u8$test)
tbl3 <- tbl_janein(u9$test)
tbl <- bind_cols(tbl1, tbl2[2:3], tbl3[2:3])
...
#***************************
u7a <- u7a %>% mutate(test = ifelse(F16 == 1 & F19 == 2, '1', '2'), 
                      test = ifelse(F16 == 0 | F19 == 0, NA, test))
u8 <- u8 %>% mutate(test = ifelse(F16 == 1 & F19 == 2, '1', '2'), 
                    test = ifelse(F16 == 0 | F19 == 0, NA, test))
u9 <- u9 %>% mutate(test = ifelse(F16 == 1 & F19 == 2, '1', '2'), 
                    test = ifelse(F16 == 0 | F19 == 0, NA, test))
...
#*****************
u7a <- u7a %>% mutate(test = ifelse(F16 == 2 & F19 == 1, '1', '2'), 
                      test = ifelse(F16 == 0 | F19 == 0, NA, test))
u8 <- u8 %>% mutate(test = ifelse(F16 == 2 & F19 == 1, '1', '2'), 
                    test = ifelse(F16 == 0 | F19 == 0, NA, test))
u9 <- u9 %>% mutate(test = ifelse(F16 == 2 & F19 == 1, '1', '2'), 
                    test = ifelse(F16 == 0 | F19 == 0, NA, test))
...
#*******************
u7a <- u7a %>% mutate(test = ifelse(F16 == 2 & F19 == 2, '1', '2'), 
                      test = ifelse(F16 == 0 | F19 == 0, NA, test))
u8 <- u8 %>% mutate(test = ifelse(F16 == 2 & F19 == 2, '1', '2'), 
                    test = ifelse(F16 == 0 | F19 == 0, NA, test))
u9 <- u9 %>% mutate(test = ifelse(F16 == 2 & F19 == 2, '1', '2'), 
                    test = ifelse(F16 == 0 | F19 == 0, NA, test))
...
tbl <- bind_rows(tmp1[1,], tmp2[1,], tmp3[1,], tmp4[1,], tmp1[3:5,])
tbl$Antwort <- c('beide Tests', 'Stereo Test', 'Formenwiedererkennungstest', 'keiner von beiden Tests', 'valides n', 'fehlend', 'gesamt n')
...
#### 4.1.4.2	Häufigkeit der Feststellung von Auffälligkeiten 
...
Stereo-Test auffällig? Nur ausgewertet, wenn Stereo-Test durchegführt.
...
Stereo-Test auffällig? Nur ausgewertet, wenn Stereo-Test *mit* binokularem Autorefraktometer durchegführt.
...
Stereo-Test auffällig? Nur ausgewertet, wenn Stereo-Test *ohne* binokularem Autorefraktometer durchegführt.
...
## 4.2	Untersuchung des Hörvermögens (U8)

### 4.2.2	Hörtest mittels Screeningaudiometrie (U8)

#### 4.2.2.1	Häufigkeit der Durchführung 
...
#### 4.2.2.2	Gründe keine Audiometrie durchzuführen
...
tbl <- read_xlsx('O:/U4951/Consulimus_Abschlussdatenlieferung/03_Sammelbögen/02_Freitexte/Freitexte_GruendeAudiometrie_CodiersystemMAXQDA_grb.xlsx', sheet='R_input')
...
#### 4.2.2.3	Häufigkeit der Feststellung von Auffälligkeiten
...
\pagebreak
#### 4.2.2.4	Häufigkeit von Überweisungen
...
## 4.3	Untersuchung der Zähne und der Schleimhaut (U6)

### 4.3.2	Auffälligkeiten an Zähnen und Schleimhaut (U6)

#### 4.3.2.1	Häufigkeit der Feststellung von Auffälligkeiten
...
#### 4.3.2.2	Häufigkeit von Überweisungen 
 \newline
...
## 4.5	Untersuchung auf Sprach- und Sprechstörungen (U5–U9)

### 4.5.2	Hinweise auf Sprach- und Sprechstörungen
...
### 4.5.5	Durchführung von Sprachberatung
...
### 4.5.6	Diagnostische und therapeutische Maßnahmen
...
### abs
txt2 %>% 
 mutate(across(U5:`Ges.`, ~replace_na(., 0))) %>% 
...
\pagebreak
## 4.7	Information und Beratung der Eltern (U2–U9)

### 4.7.2	Feststellung von erweitertem Beratungsbedarf (U2–U9)
...
# 5. Ergebnisse der Evaluation von Anlage 1 (Untersuchungsheft für Kinder) der Kinder-Richtlinie

## 5.2	Vorliegen des Gelben Heftes bei U-Untersuchungen
...
## 5.3	Häufigkeit der Dokumentation im Gelben Heft

### 5.3.1	Dokumentation von Auffälligkeiten 

#### 5.3.1.1 Auffälligkeiten im Abschnitt *Untersuchung* insgesamt 
...
#### 5.3.1.3	Auffälligkeiten im Abschnitt *Ergebnisse*
...
### 5.3.2	Dokumentation von erweitertem Beratungsbedarf 

#### 5.3.2.1	Erweiterter Beratungsbedarf insgesamt
 \newline
Wie viele Beratungsthemen mit erweitertem Beratungsbedarf wurden im Abschnitt *Beratung* angekreuzt?
```{r echo=FALSE, message=FALSE, warning=FALSE}
...
## 5.5	Aufwand für die Dokumentation 
...
## 5.6	Nutzung der Teilnahmekarte

### 5.6.1	Ausfüllen der Teilnahmekarte
