...
tbl_janein <-function(x) {
  ...
  tbl2 <- as.data.frame(round(prop.table(table(x)),3)*100)
  #tbl2$Freq <- paste(tbl2$Freq, '%') #doch kein %-Zeichen
  tbl2 <- rbind(tbl2, NA)
  tbl1 <- cbind(tbl1, tbl2[2])
  # zusätzliche Zeilen einfügen
  tbl1[4,] <- tbl1[3,] #NAs verschieben
  tbl1[3,2:3] <- colSums(tbl1[1:2,2:3]) #valides n
  tbl1[5,2] <- sum(tbl1[3:4,2]) #gesamt n
  tbl1
}
...
ges_janein <- function(df){
  ...
  a <- df$`Ges. [n]`[1] / df$`Ges. [n]`[3]*100
  b <- df$`Ges. [n]`[2] / df$`Ges. [n]`[3]*100
  c <- sum(a+b)
  df$`Ges. [%]` <- as.numeric(c(a,b,c, NA, NA))
  df
}
...
## 3. Ergebnisse zu Frühereknnungsuntersuchungen

### 3.1	Untersuchung der Augen 

#### 3.1.2 Inspektion und Prüfung im durchfallenden Licht
...
#tbl$gesamt <- tbl[2] + tbl[4] + tbl[6]

tbl$x <- rownames_janein
names(tbl) <- c('Antwort','U2 [n]','U2 [%]','U3 [n]','U3 [%]')

tbl <- ges_janein(tbl)

tbl %>% kbl2(booktabs = T, digits=1, centering=F) %>% # Für den Tabellenstil
...
tbl %>% kbl2(booktabs = T, digits=1) %>% # Für den Tabellenstil
  kable_styling(latex_options = "HOLD_position", position = "left") %>% # Zwingt die Tabelle an die Position im Text
  row_spec(2, extra_css = "border-bottom: 1px solid", hline_after=TRUE)
```
...
tbl2 <- tbl_janein(u3$F13[(u3$F9 == 1 &  u3$F10 == 1) | (u3$F11 == 1 &  u2$F12 == 1)])
tbl <- bind_cols(tbl1, tbl2[2:3])
...
tbl$x <- rownames_janein
names(tbl) <- c('Antwort','U4 [n]','U4 [%]','U5 [n]','U5 [%]','U6 [n]','U6 [%]','U7 [n]','U7 [%]')
...
Transilluminationsunterschied festgestellt? Nur ausgewertet, wenn ein Brückner-Test *ohne* binokularen Autorefraktometer durchgeführt wurde.
```{r echo=FALSE, message=FALSE, warning=FALSE}
...
tbl$x <- rownames_janein
names(tbl) <- c('Antwort','U7a [n]','U7a [%]','U8 [n]','U8 [%]','U9 [n]','U9 [%]')
...
Test mit einem binokularen Autorefraktometer durchgeführt? (z. B. von Plusoptix, Welch Allyn o. a. Herstellern). Nur ausgewertet, wenn Hornhautreflexbildchen durchgeführt. 
```{r echo=FALSE, message=FALSE, warning=FALSE}
...
Strabismus festgestellt? Nur ausgeweret, wenn Hornhautreflexbildchen *ohne* binokularen Autorefraktometer durchgeführt
```{r echo=FALSE, message=FALSE, warning=FALSE}
...
### 3.2	Untersuchung des Hörvermögens
#### 3.2.2	Hörtest mittels Screeningaudiometrie (U8)
##### 3.2.2.1	Häufigkeit der Durchführung 

 \newline
Prüfung mittels Tonschwellen-Audiometrie durchgeführt?
```{r echo=FALSE, message=FALSE, warning=FALSE}
tbl <- tbl_janein(u8$F21)
tbl$x <- rownames_janein
names(tbl) <- c('Antwort','U8 [n]','U8 [%]')
...
 \newline
Falls Prüfung mittels Tonschwellen-Audiometrie NICHT durchgeführt: Warum nicht? (Mehrfachnennungen möglich). Nur ausgewertet, wenn keine Prüfung mittels Tonschwellen-Audiometrie durchgeführt.
...
tbl %>% 
  slice_head(n=7) %>%
  kbl2(booktabs = T, escape = F, digits=1, 
       col.names=c("Grund", "n", "\\% Ärzte/Ärztinnen")) %>%
    kable_styling(latex_options = "HOLD_position", position = "left")
```
  \linebreak
...
 \newline
Tonschwellen-Audiometrie mit auffälligem Befund? Nur ausgewertet, wenn eine Prüfung mittels Tonschwellen-Audiometrie durchgeführt wurde. 
```{r echo=FALSE, message=FALSE, warning=FALSE}
...
Bei auffälligem Befund: Gab es vor der heutigen Audiometrie bereits Anhaltspunkte für eine Störung des Hörvermögens? Nur ausgewertet, wenn eine Tonschwellen-Audiometrie durchgeführt und ein auffälliger Befund angegeben wurde. 
```{r echo=FALSE, message=FALSE, warning=FALSE}
...
 \newline
Bei auffälligem Befund: Überweisung an HNO-Arzt oder Pädaudiologen? Nur ausgewertet, wenn eine Tonschwellen-Audiometrie durchgeführt und ein auffälliger Befund angegeben wurde
```{r echo=FALSE, message=FALSE, warning=FALSE}
...
Bei auffälligem Befund: Überweisung an HNO-Arzt oder Pädaudiologen? Nur ausgewertet, wenn eine Tonschwellen-Audiometrie durchgeführt und ein **erstmaliger** auffälliger Befund angegeben wurde
```{r echo=FALSE, message=FALSE, warning=FALSE}
...
### 3.3	Untersuchung der Zähne und der Schleimhaut
...
### 3.6	Untersuchung auf Sprach- und Sprechstörungen (U5–U9)
#### 3.6.2	Hinweise auf Sprach- und Sprechstörungen
...
#### 3.6.5	Durchführung von Sprachberatung
...
#### 3.6.6	Diagnostische und therapeutische Maßnahmen
...
### 3.7	Information und Beratung der Eltern
...
## 4. Ergebnisse zum Untersuchungsheft für Kinder
### 4.1	Vorliegen des Gelben Heftes bei U-Untersuchungen
...
### 4.2	Häufigkeit der Dokumentation 
#### 4.2.1	Dokumentation von Auffälligkeiten 
##### 4.2.1.1	Auffälligkeiten insgesamt 
...
#### 4.2.2	Dokumentation von erweitertem Beratungsbedarf 
...
**Die Frage ist bereits unter 3.7.2 ausgeweret**


#### 4.2.3	Dokumentation von Auffälligkeiten zur Beobachtung
##### 4.2.3.1	Auffälligkeiten zur Beobachtung insgesamt 
...
### 4.4	Aufwand für die Dokumentation 
Zeitaufwand für Eintragungen in das Gelbe Heft (Schätzwert in Minuten)
```{r echo=FALSE, message=FALSE, warning=FALSE}
tbl1 <- tbl_kont(u2$F8)
tbl2 <- tbl_kont(u3$F8)
tbl3 <- tbl_kont(u4$F8)
...
Anzahl genannter Zeitaufwände: 
```{r echo=FALSE, message=FALSE, warning=FALSE}
u2 <- u2 %>% mutate(F8_cat=case_when(F8 == 0 ~ '0', F8 < 1 ~ '<1', F8 < 2 ~ '>=1 & <2', 
                                     F8 < 5 ~ '>=2 & <5', F8 < 10 ~ '>=5 & <10', F8 >= 10 ~ '>=10'))
u3 <- u3 %>% mutate(F8_cat=case_when(F8 == 0 ~ '0', F8 < 1 ~ '<1', F8 < 2 ~ '>=1 & <2', 
                                     F8 < 5 ~ '>=2 & <5', F8 < 10 ~ '>=5 & <10', F8 >= 10 ~ '>=10'))
u4 <- u4 %>% mutate(F8_cat=case_when(F8 == 0 ~ '0', F8 < 1 ~ '<1', F8 < 2 ~ '>=1 & <2', 
                                     F8 < 5 ~ '>=2 & <5', F8 < 10 ~ '>=5 & <10', F8 >= 10 ~ '>=10'))
u5 <- u5 %>% mutate(F8_cat=case_when(F8 == 0 ~ '0', F8 < 1 ~ '<1', F8 < 2 ~ '>=1 & <2', 
                                     F8 < 5 ~ '>=2 & <5', F8 < 10 ~ '>=5 & <10', F8 >= 10 ~ '>=10'))
u6 <- u6 %>% mutate(F8_cat=case_when(F8 == 0 ~ '0', F8 < 1 ~ '<1', F8 < 2 ~ '>=1 & <2', 
                                     F8 < 5 ~ '>=2 & <5', F8 < 10 ~ '>=5 & <10', F8 >= 10 ~ '>=10'))
u7 <- u7 %>% mutate(F8_cat=case_when(F8 == 0 ~ '0', F8 < 1 ~ '<1', F8 < 2 ~ '>=1 & <2', 
                                     F8 < 5 ~ '>=2 & <5', F8 < 10 ~ '>=5 & <10', F8 >= 10 ~ '>=10'))
u7a <- u7a %>% mutate(F8_cat=case_when(F8 == 0 ~ '0', F8 < 1 ~ '<1', F8 < 2 ~ '>=1 & <2', 
                                     F8 < 5 ~ '>=2 & <5', F8 < 10 ~ '>=5 & <10', F8 >= 10 ~ '>=10'))
u8 <- u8 %>% mutate(F8_cat=case_when(F8 == 0 ~ '0', F8 < 1 ~ '<1', F8 < 2 ~ '>=1 & <2', 
                                     F8 < 5 ~ '>=2 & <5', F8 < 10 ~ '>=5 & <10', F8 >= 10 ~ '>=10'))
u9 <- u9 %>% mutate(F8_cat=case_when(F8 == 0 ~ '0', F8 < 1 ~ '<1', F8 < 2 ~ '>=1 & <2', 
                                     F8 < 5 ~ '>=2 & <5', F8 < 10 ~ '>=5 & <10', F8 >= 10 ~ '>=10'))
...
### 4.5	Teilnahmekarte
#### 4.5.1	Ausfüllen der Teilnahmekarte
